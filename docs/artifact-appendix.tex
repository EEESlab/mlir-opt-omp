...
\begin{itemize}
  ...
  \item {\bf Program: } the PolyBench/C 4.2.1 OpenMP kernels, taken from a
    checkout the artifact is pointed at (\texttt{POLYBENCH}); and 74
  ...
  \item {\bf Run-time environment: } Linux x86\_64 (Ubuntu 22.04/24.04) with
    ...
    the runtime's own defaults for thread binding and wait policy.  The
    \texttt{pmsis} target additionally needs the
  ...
  \item {\bf Run-time state: } the machine must be otherwise idle.
  ...
  \item {\bf Experiments: } (i) the regression suite (\texttt{check-omp});
    ...
    (iv) \texttt{constructs/run\_constructs.sh} for the constructs and clauses
    PolyBench never writes; each of (ii)--(iv)
  ...
  \item {\bf How much time is needed to complete experiments (approximately)?: }
    under a minute for the regression suite, and about TODO hours for the
    PolyBench suite at
...
and the integration drivers --- roughly
...
PolyBench/C 4.2.1 with OpenMP pragmas.  No kernels are vendored in the
artifact: \texttt{POLYBENCH} must point at a checkout, and the drivers refuse
to start otherwise rather than silently measuring nothing.  Inputs are
generated by PolyBench itself from the selected dataset macro; no external data
files are needed.
...
\begin{verbatim}
cd test/Integration
./run_correctness.sh                    # iomp, whole suite
...
\noindent {\bf E4 --- constructs and clauses.}
\texttt{constructs/run\_constructs.sh} (with \texttt{RUNTIME=iomp} and
\texttt{libgomp}): one standalone C program per construct/clause, printing
\texttt{42} only if the clause took effect.  This is the only end-to-end
evidence for most of Table~\ref{tab:support}: over the whole PolyBench suite
only \texttt{parallel}, a bare \texttt{for} and \texttt{private} ever appear,
so \texttt{firstprivate}, \texttt{num\_threads}, \texttt{proc\_bind},
\texttt{nowait}, an explicit \texttt{schedule}, \texttt{if}, \texttt{barrier},
\texttt{task} and \texttt{taskwait} are exercised nowhere else.
...
\texttt{pmsis}), an optional explicit \texttt{KERNELS} list (empty runs the
whole suite), \texttt{DATASET}, \texttt{THREADS},
