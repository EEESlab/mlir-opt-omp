...
// `construct wsloop when schedule == static` for iomp (and for libgomp), so no
// construct matches and the DSL evaluation fails.
//
...
//
// NOTE the asymmetry: pmsis declares an UNGUARDED `construct wsloop`, so there
// the same input matches and is lowered as a static block distribution with no
// diagnostic at all.  See test/README.md.
