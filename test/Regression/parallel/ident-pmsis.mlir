...
// Like libgomp, the pmsis path (ext_pi_cl_* API) must NOT emit any ident_t or
// psource global, even when run through the full outlining pipeline.
...
//
// A second pass with its own prefix anchors the absence checks below: on their
// own they would also hold for an empty module, so they cannot tell "no ident
// because pmsis needs none" from "no ident because nothing was lowered".
// RUN: mlir-opt-omp %s --omp-lower-dsl=%rules_dsl --omp-lower-runtime=pmsis \
